...
export async function generatePDF(env: Env): Promise<Response> { 
  ...
  const htmlContent = `
    ...
    <html>
      ...
      <body>
        ...
        <div class="header">
          <h1 id="title">15 Principles for Managing Up</h1>
          <p id="description">A practical guide to choosing the best text-to-speech model for your project based on quality, cost, and reliability.</p>
          <p id="byline">By Louise Perry • Apr 20, 2025</p>
        </div>
    ...
    fetchAndProcessUsers('https://jsonplaceholder.typicode.com/users'); // Placeholder API</code></pre>
        ...
        <div class="footer">
          <span>Made with Article to PDF</span>
        </div>
      </body>
    </html>
  `;
	...
	try {
		browser = await puppeteer.launch(env.PDF_BROWSER); 
		...
		return new Response(pdfBuffer, {
			headers: {
				...
				'Content-Disposition': 'inline; filename="generated-article.pdf"', // 'inline' tries to display in browser, 'attachment' forces download
			},
		});
	} catch (error) {
...
- gör bara content utand header i llm reponse.
- gör en brandad footer or last section after content with company logo.

Deal med alla typer of content.
1. text
2. images
        <figure>
